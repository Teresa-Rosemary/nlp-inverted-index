...
# Replace the input dataset filepath
lines = sc.wholeTextFiles(inpath)

# Flatten the values by splitting the text files into normalised tokens
flattenToWords = lines.flatMapValues(tokenize).distinct()
...
if __name__ == "__main__":
    ...
    wordId.map(lambda row: str(row[0]) + " "*(30-len(row[0])) + str(row[1])).saveAsTextFile(dictpath)
    ...
    invertedIndex.map(lambda row: str(row[0]) + "\t" + str(row[1])).saveAsTextFile(indexpath)
